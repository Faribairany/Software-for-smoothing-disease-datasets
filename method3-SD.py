...
import math
from collections import defaultdict 
with open('(meanabsoluteerror+std)(method3).csv', mode='w', newline='') as csv_file:
            fieldnames = ['Filename','Mean Error(Method3)', 'Standard Deviation(Method3)']
            writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
            writer.writeheader()
path = "C:/Users/Fariba Afrin Irany/Desktop/study/methods(SD included)/method3/inputfile"
...
for filename in os.listdir(path):
    ...
    adjusterror=0
    df1 = pd.read_csv("C:/Users/Fariba Afrin Irany/Desktop/study/methods(SD included)/method3/inputfile/{}".format(filename))

    # stores the method2reportedcolumn column value
    listpopulationmaincdcfile = df1['Method3(Pop+local risk)'].tolist()
    listerror=df1['Error(Method3(Pop+local_risk))'].tolist()
    # print(listerror)
    ...
    df1['Method3(Pop+local risk)'] = method2reportedcolumn

    df1['Error(Method3(Pop+local_risk))'] =  method2error
    x = filename.split(".")
...
#    dict with keys (county code, age group) and value=values need to be adjusted
    ...
    df1['Method3(Pop+local risk)(adjusted)'] = overunder
    ...
    df1 = df1.drop('Method3(Pop+local risk)', axis=1)
    df1.to_csv(outputfile, mode='w', index=False)

    df1 = df1.drop('Error(Method3(Pop+local_risk))', axis=1)
    df1.to_csv(outputfile, mode='w', index=False)
    #list to keep the difference column


    df1 = df1.drop('Method3(Pop+local risk)(adjusted)', axis=1)
    df1.to_csv(outputfile, mode='w', index=False)
    #list to keep the difference column


    
    df1['Method3(Pop+local Risk)'] = overunder
   ...
   # print(adjusterror)
    ...
    method2list = df1['Method3(Pop+local Risk)'].tolist()
    ...
    df1['Method3(Pop+local Risk)'] =  methodvallist
    df1.to_csv(outputfile, mode='w', index=False)
    ...
    df1['Error(Method3(Pop+local_risk))']=listerroradj
    df1.to_csv(outputfile, mode='w', index=False)
    ...
    # # print(avgerror)
    with open('(meanabsoluteerror+std)(method3).csv', 'a', newline='') as file:
